...
def G_arch(ch=64, attention='64', ksize='333333', dilation='111111'):
    ...
    arch[128] = {'in_channels':  [ch * item for item in [16, 16, 8, 4, 2]],
                 ...
                 'attention': {2**i: (2**i in [int(item) for item in attention.split('_')])
                               for i in range(3, 8)}}
    arch[64] = {'in_channels':  [ch * item for item in [16, 8, 4, 2]],
...
class Generator(nn.Module):
    def __init__(self, G_ch=64, dim_z=128, bottom_width=4, resolution=128,
        ...
        self.which_embedding = functools.partial(nn.Linear, bias=False)
        # self.which_embedding = functools.partial(self.which_linear, bias=False)
...
class G_D(nn.Module):
    ...
    def forward(self, z, gy, x=None, dy=None, train_G=False, return_G_z=False, policy=False, CR=False, CR_augment=None, mixup=False):
        if z is not None:
        ...
        if False and mixup:
            D_input, D_class = utils.mixup(D_input, D_class, 0.2)
        if CR:
            if CR_augment:
