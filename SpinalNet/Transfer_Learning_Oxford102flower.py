...
val_dataset = torch.utils.data.ConcatDataset(
    [datasets.ImageFolder(os.path.join(data_dir, x), data_transforms['val']) for x in ['', '']])
train_dataset = datasets.ImageFolder(os.path.join(data_dir, ''), data_transforms['train'])
image_datasets = {'val': val_dataset, 'train': train_dataset}
image_datasets['train'], image_datasets['val'] = image_datasets['val'], image_datasets['train']
# image_datasets['val'], _ = torch.utils.data.random_split(image_datasets['val'], [552, 6000])
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                if phase == 'train':
                    ...
                    if batch_iter % save_every == 0:
                        ...
                        torch.save(model.state_dict(), f'model_oxford102flower_iter{batch_iter}.pth')
                        model.train()
...
def predict_iter(model, batch_iter):
    dataset = ImageFolderWithPath(os.path.join(data_dir, ''), data_transforms['val'])
    ...
    np.savez(f'oxford102_iter{batch_iter}.npz', predicts, paths, ls)
...
def predict(model):
    ...
    print(predicts.shape)
    np.savez('oxford102_softmax.npz', predicts, paths)
...
model_ft = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler,
                      num_epochs=10)
torch.save(model_ft.state_dict(), 'model_oxford102.pth')
# model_ft.load_state_dict(torch.load('model_oxford102.pth'))
model_ft.eval()
predict(model_ft)
