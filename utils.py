...
def prepare_parser():
    ...
    parser.add_argument('--use_balanced_sampler', action="store_true", help='use class balanced sampling')
    parser.add_argument('--reweighting', action='store_true', default=False, help='use reweigthing loss')
    parser.add_argument('--target_type', type=str, default='onehot', choices=['onehot', 'softmax', 'smooth', 'ols'], help='select label type')
    parser.add_argument('--ann_file', type=str, default=None)
    parser.add_argument('--classifier', type=str, default=None,
                        help='Which classifier to evaluate')
    parser.add_argument('--sample_order', default=None, type=str)
    parser.add_argument('--smooth_alpha', default=0.1, type=float)
    parser.add_argument('--balanced_alpha', default=1.0, type=float)
    return parser
...
# Convenience dicts
dset_dict = {'I32': dset.SoftmaxDataset, 'I64': dset.SoftmaxDataset,
             'I128': dset.SoftmaxDataset, 'I256': dset.SoftmaxDataset,
             'I32_hdf5': dset.ILSVRC_HDF5, 'I64_hdf5': dset.ILSVRC_HDF5,
             'I128_hdf5': dset.ILSVRC_HDF5, 'I256_hdf5': dset.ILSVRC_HDF5,
             'F32': dset.SoftmaxDataset, 'F64': dset.SoftmaxDataset,
             'F128': dset.SoftmaxDataset, 'F256': dset.SoftmaxDataset,
             'C10': dset.SoftmaxCIFAR10, 'C100': dset.SoftmaxCIFAR100,
             'S64': dset.SoftmaxDataset, 'S128': dset.SoftmaxDataset, 'S256': dset.SoftmaxDataset,
             'T64': dset.SoftmaxDataset, 'T128': dset.SoftmaxDataset, 'T256': dset.SoftmaxDataset,
             'L64': dset.SoftmaxDataset, 'L128': dset.SoftmaxDataset, 'L256': dset.SoftmaxDataset
             }
...
imsize_dict = {'I32': 32, 'I32_hdf5': 32,
               ...
               'C10': 32, 'C100': 32,
               'S64': 64, 'S128': 128, 'S256': 256,
               'T64': 64, 'T128': 128, 'T256': 256,
               'L64': 64, 'L128': 128, 'L256': 256}
root_dict = {'I32': 'animeface-character-dataset/train', 'I32_hdf5': 'ILSVRC32.hdf5',
             'I64': 'animeface-character-dataset/train', 'I64_hdf5': 'ILSVRC64.hdf5',
             'I128': 'animeface-character-dataset/train', 'I128_hdf5': 'ILSVRC128.hdf5',
             'I256': 'animeface-character-dataset/train', 'I256_hdf5': 'ILSVRC256.hdf5',
             'F32': 'flower', 'F64': 'flower', 'F128': 'flower', 'F256': 'flower',
             'C10': 'cifar', 'C100': 'cifar',
             'S64': 'car_data/train', 'S128': 'car_data/train', 'S256': 'car_data/train',
             'T64': 'TINY_ILSVRC2012/train', 'T128': 'TINY_ILSVRC2012_128/train',
             'T256': 'TINY_ILSVRC2012_256/train',
             'L64': 'ImageNet_LT/train', 'L128': 'ImageNet_LT/train',
             'L256': 'ImageNet_LT/train'}
val_root_dict = {'I32': 'animeface-character-dataset/train', 'I32_hdf5': 'ILSVRC32.hdf5',
                 'I64': 'animeface-character-dataset/train', 'I64_hdf5': 'ILSVRC64.hdf5',
                 'I128': 'animeface-character-dataset/train', 'I128_hdf5': 'ILSVRC128.hdf5',
                 'I256': 'animeface-character-dataset/train', 'I256_hdf5': 'ILSVRC256.hdf5',
                 'F32': 'flower', 'F64': 'flower', 'F128': 'flower', 'F256': 'flower',
                 'C10': 'cifar', 'C100': 'cifar',
                 'S64': 'car_data/test', 'S128': 'car_data/test', 'S256': 'car_data/test',
                 'T64': 'TINY_ILSVRC2012/valid', 'T128': 'TINY_ILSVRC2012_128/valid',
                 'T256': 'TINY_ILSVRC2012_256/valid',
                 'L64': 'ImageNet_LT/valid', 'L128': 'ImageNet_LT/valid',
                 'L256': 'ImageNet_LT/valid'}
nclass_dict = {'I32': 176, 'I32_hdf5': 1000,
               'I64': 176, 'I64_hdf5': 1000,
               'I128': 176, 'I128_hdf5': 1000,
               'I256': 176, 'I256_hdf5': 1000,
               'F32': 102, 'F64': 102, 'F128': 102, 'F256': 102,
               'C10': 10, 'C100': 100,
               'S64': 196, 'S128': 196, 'S256': 196,
               'T64': 200, 'T128': 200, 'T256': 200,
               'L64': 1000, 'L128': 1000, 'L256': 1000}
# Number of classes to put per sample sheet
classes_per_sheet_dict = {'I32': 50, 'I32_hdf5': 50,
                          'I64': 50, 'I64_hdf5': 50,
                          'I128': 20, 'I128_hdf5': 20,
                          'I256': 8, 'I256_hdf5': 20,
                          'F32': 50, 'F64': 50, 'F128': 20, 'F256': 10,
                          'C10': 10, 'C100': 100,
                          'S64': 49, 'S128': 20, 'S256': 10,
                          'T64': 20, 'T128': 20, 'T256': 10,
                          'L64': 20, 'L128': 20, 'L256': 10}
...
def partial_mixup(input: torch.Tensor,
    ...
    perm_input = input[indices] #.detach()
...
def make_weights_for_balanced_classes(dataset, alpha=1.0):
    n_samples = 0
    ...
    weights = np.array([c / n_samples for c in counts])
    uweights = np.repeat(1 / n_classes, n_classes)
    weights = (1 - alpha) * weights + alpha * uweights
    rates = torch.tensor([1/(weights[c]*counts[c]) for c in classes])
...
def get_data_loaders(dataset, data_root=None, mirror_augment=False, batch_size=64,
                     ...
                     use_balanced_sampler=False, ann_file=None, c=None,
                     **kwargs):

    # Append /FILENAME.hdf5 to root if using hdf5
    data_root += '/%s' % (root_dict[dataset] if train else val_root_dict[dataset])
    ...
    dataset_kwargs = {'index_filename': '%s_%s_imgs.npz' % (dataset, ('train' if train else 'valid')), 'smooth_alpha': kwargs['smooth_alpha']}
    ...
    if 'hdf5' in dataset:
        ...
        if mirror_augment:
            ...
            train_transform = [transforms.Resize((image_size, image_size)), transforms.RandomHorizontalFlip()]
        else:
            print('Data will not be augmented...')
            train_transform = []
            train_transform = [transforms.Resize((image_size, image_size))] #, transforms.CenterCrop]
        train_transform = transforms.Compose(train_transform + [
            transforms.ToTensor(),
            transforms.Normalize(norm_mean, norm_std)])
    train_set = which_dataset(root=data_root, transform=train_transform, seed=seed, target_type=target_type,
                              load_in_mem=load_in_mem, num_samples=num_samples, train=train, ann_file=ann_file, **dataset_kwargs)

    if c is not None:
        class_indices = train_set.get_indices(c)
        train_set = Subset(train_set, class_indices)
    ...
    if use_multiepoch_sampler:
        ...
        weights = make_weights_for_balanced_classes(train_set, kwargs['balanced_alpha'])
        sampler = WeightedRandomSampler(weights, replacement=True, num_samples=len(train_set))
        train_loader = DataLoader(train_set, batch_size=batch_size,
                                  sampler=sampler, **loader_kwargs)
    else:
        loader_kwargs = {'num_workers': num_workers, 'pin_memory': pin_memory,
...
def sample(G, z_, y_, config):
    with torch.no_grad():
        z_.sample_()
        y_.sample_()
        label = torch.eye(y_.num_categories, device=y_.device)[y_]
        if config['parallel']:
            G_z = nn.parallel.data_parallel(G, (z_, G.shared(label)))
        else:
            G_z = G(z_, G.shared(label))
        return G_z, y_


def sample_y(G, z_, num_classes, y_, config):
    with torch.no_grad():
        z_.sample_()
        label = torch.eye(num_classes)[torch.ones(z_.shape[0]).long()*y_].to(z_.device)
        if config['parallel']:
            G_z = nn.parallel.data_parallel(G, (z_, G.shared(label)))
        else:
            G_z = G(z_, G.shared(label))
        return G_z, torch.ones(z_.shape[0]).long()*y_
...
def sample_sheet(G, classes_per_sheet, num_classes, samples_per_class, parallel,
    ...
    for i in range(num_classes // classes_per_sheet):
        ...
        y = torch.arange(i * classes_per_sheet, (i + 1) *
                         classes_per_sheet, device=device)
        for j in range(samples_per_class):
            ...
            with torch.no_grad():
                label = torch.eye(num_classes)[y].to(y.device)
                if parallel:
                    o = nn.parallel.data_parallel(
                        G, (z_[:classes_per_sheet], G.shared(label)))
                else:
                    o = G(z_[:classes_per_sheet], G.shared(label))
...
# Orderd Sample function for sample sheets
def ordered_sample_sheet(G, classes_per_sheet, num_classes, samples_per_class, parallel,
                         samples_root, experiment_name, folder_number, order, z_=None, device='cuda'):
    # Prepare sample directory
    if not os.path.isdir('%s/%s' % (samples_root, experiment_name)):
        os.mkdir('%s/%s' % (samples_root, experiment_name))
    if not os.path.isdir('%s/%s/%d' % (samples_root, experiment_name, folder_number)):
    ...
    # loop over total number of sheets
    i = 0
    while i*classes_per_sheet <= num_classes:
        ims = []
        c_sheet = min(classes_per_sheet, num_classes - i*classes_per_sheet)
        y = torch.tensor(order[i*classes_per_sheet:i*classes_per_sheet+c_sheet], device=device)

        for j in range(samples_per_class):
            if (z_ is not None) and hasattr(z_, 'sample_') and classes_per_sheet <= z_.size(0):
                z_.sample_()
            else:
                z_ = torch.randn(c_sheet, G.dim_z, device=device)
            with torch.no_grad():
                label = torch.eye(num_classes)[y].to(y.device)
                if parallel:
                    o = nn.parallel.data_parallel(
                        G, (z_[:c_sheet], G.shared(label)))
                else:
                    o = G(z_[:c_sheet], G.shared(label))
        ...
        out_ims = torch.stack(ims, 1).view(-1, ims[0].shape[1], ims[0].shape[2],
                                           ims[0].shape[3]).data.float().cpu()
        # The path for the samples
        image_filename = '%s/%s/%d/ordered_samples%d.jpg' % (samples_root, experiment_name,
                                                     folder_number, i)
        torchvision.utils.save_image(out_ims, image_filename,
                                     nrow=samples_per_class, normalize=True)
        i += 1
...
def fixed_sample_sheet(G, classes_per_sheet, num_classes, samples_per_class, parallel,
                       samples_root, experiment_name, folder_number, fixed_z, device='cuda'):
    # Prepare sample directory
    if not os.path.isdir('%s/%s' % (samples_root, experiment_name)):
        os.mkdir('%s/%s' % (samples_root, experiment_name))
    if not os.path.isdir('%s/%s/%d' % (samples_root, experiment_name, folder_number)):
    ...
    for i in range(num_classes // classes_per_sheet):
        ...
        for j in range(samples_per_class):
            with torch.no_grad():
                ...
                if parallel:
                    o = nn.parallel.data_parallel(
                        G, (fixed_z[j].repeat(classes_per_sheet, 1), G.shared(label)))
                else:
                    o = G(fixed_z[j].repeat(classes_per_sheet, 1), G.shared(label))
        ...
        out_ims = torch.stack(ims, 1).view(-1, ims[0].shape[1], ims[0].shape[2],
                                           ims[0].shape[3]).data.float().cpu()
        # The path for the samples
        image_filename = '%s/%s/%d/fixed_samples%d.jpg' % (samples_root, experiment_name,
                                                     folder_number, i)
        torchvision.utils.save_image(out_ims, image_filename,
                                     nrow=samples_per_class, normalize=True)
...
def interp(x0, x1, num_midpoints):
    lerp = torch.linspace(0, 1.0, num_midpoints + 2,
                          device='cuda').to(x0.dtype).to(x0.device)
...
def interp_sheet(G, num_per_sheet, num_midpoints, num_classes, parallel,
    ...
    if fix_y:  # If fix y, only sample 1 z per row
        ys = sample_1hot(num_per_sheet, num_classes, device=device)
        ys = G.shared(ys).view(num_per_sheet, 1, -1)
        ys = ys.repeat(1, num_midpoints + 2,
                       1).view(num_per_sheet * (num_midpoints + 2), -1)
    else:
        ys = interp(G.shared(sample_1hot(num_per_sheet, num_classes, device=device)).view(num_per_sheet, 1, -1),
                    G.shared(sample_1hot(num_per_sheet, num_classes, device=device)).view(
                        num_per_sheet, 1, -1),
    ...
    with torch.no_grad():
        label = ys
        # label = torch.eye(num_classes)[ys].to(ys.device)
        if parallel:
            out_ims = nn.parallel.data_parallel(G, (zs, label)).data.cpu()
        else:
            out_ims = G(zs, label).data.cpu()
...
def sample_1hot(batch_size, num_classes, device='cuda'):
    return torch.eye(num_classes)[torch.randint(low=0, high=num_classes, size=(batch_size,), device=device, dtype=torch.int64, requires_grad=False)].to(device)
